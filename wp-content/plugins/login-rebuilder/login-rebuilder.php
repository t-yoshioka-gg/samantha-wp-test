...
/*
 Copyright (C) 2013-2022 tmatsuur (Email: takenori dot matsuura at 12net dot jp)
...
define( 'LOGIN_REBUILDER_DB_VERSION', '2.8.0' );
		...
		foreach ( $users as $user ) {
			...
			if ( is_array( $session_tokens ) ) {
				...
				foreach ( $session_tokens as $token=>$session ) {
					...
					$ua = isset( $session['ua'] )? $this->_parse_ua( $session['ua'] ): array( 'os' => '-', 'browser' => '-' );
					$content['users'][] = array(
						'text' => date_i18n( $date_time_format, $session['login'] + $gmt_offset * 3600 ) . ' - ' .
				...
				$content['users'][] = array(
					'text' => __( '(Unknown)', LOGIN_REBUILDER_DOMAIN ) . ' - ' .
							urldecode( $nicename ) . __( '(Unknown)', LOGIN_REBUILDER_DOMAIN ),
					'alert' => false
				);
			}
		}
